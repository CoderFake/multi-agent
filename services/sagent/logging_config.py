"""Logging configuration for the ADK backend.
...
def get_log_level(setting_value: str, default: str = "INFO") -> str:
    """Validate a log level string."""
    level = (setting_value or default).upper()
...
def configure_logging() -> None:
    ...
    """
    log_level = get_log_level(settings.LOG_LEVEL, "INFO")
    adk_log_level = get_log_level(settings.ADK_LOG_LEVEL or log_level, log_level)
    ...
    startup_logger = logging.getLogger("agent.startup")
...
def get_logging_status() -> dict:
    ...
    return {
        ...
        "env": {
            "LOG_LEVEL": settings.LOG_LEVEL or "INFO (default)",
            "ADK_LOG_LEVEL": settings.ADK_LOG_LEVEL or "uses LOG_LEVEL",
        },
    }
