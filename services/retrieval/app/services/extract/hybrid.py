...
class HybridExtractor(BaseExtractor):
    ...
    async def extract(self, file_path: str, storage_key: str) -> ExtractResult:
        ...
        if ext in DOCLING_ONLY_EXTS:
            ...
            result = await self._get_docling().extract(file_path, storage_key)
        ...
        if ext in PADDLE_ONLY_EXTS:
            ...
            result = await self._get_paddle().extract(file_path, storage_key)
        ...
        try:
            docling_result = await self._get_docling().extract(file_path, storage_key)
        ...
        paddle_result = await self._get_paddle().extract(file_path, storage_key)
