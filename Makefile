.PHONY: \
	...
	services-up services-down services-up-infra \
	rag-sync \
	lint fmt check \
...
help:
	...
	@printf "\n\033[1;34mMCP Servers\033[0m\n"
	@printf "  make mcp-pull     Pull all stdio MCP Docker images declared in mcp.json files\n"
	@printf "  make mcp-list     List all stdio MCP Docker images (dry-run, no pull)\n"
...
dev-agent:
	...
	@cd services/sagent && uv run uvicorn main:app --reload --port 8000
...
dev-agent-debug:
	...
	@cd services/sagent && LOG_LEVEL=DEBUG uv run uvicorn main:app --reload --port 8000

# Run the retrieval microservice on host (for local dev/testing)
dev-retrieval:
	@printf "\n[Retrieval] Starting on http://localhost:8001\n\n"
	@cd services/retrieval && uv run uvicorn main:app --reload --port 8001

# Install retrieval dependencies
install-retrieval:
	@printf "\n[Install] Retrieval dependencies...\n\n"
	@cd services/retrieval && uv sync
...
db-up:
	...
	@printf " ready!\n"
	@printf "\nPostgres available at: postgresql://agent:localdev@localhost:5432/agent\n"
...
LOCAL_DATABASE_URL := postgresql://agent:localdev@localhost:5432/agent

# Apply migrations to local Postgres (Drizzle schema, ADK tables auto-created)
db-migrate:
	@printf "\n[DB] Running migrations on local Postgres...\n\n"
	@printf "Drizzle migrations...\n"
...
db-reset:
	...
	@docker exec agent-postgres psql -U agent -d agent -c "\
		...
		GRANT ALL ON SCHEMA public TO agent;" > /dev/null
...
# MCP Servers
# ==============================================================================
# Agents using stdio transport run MCP servers as Docker subprocesses.
# The Docker images must be pulled before starting the agent.
#
# Convention: each mcp.json with {"type": "stdio", "command": "docker", ...}
#             declares a Docker image to pull.
#
# Usage:
#   make mcp-list     - Show all stdio MCP Docker images (no pull)
#   make mcp-pull     - Pull all stdio MCP Docker images
#
# Run once on first setup, then whenever an agent adds a new MCP server.
# ==============================================================================

# List stdio MCP Docker images (dry-run, no pull)
mcp-list:
	@printf "\n[MCP] Scanning stdio MCP Docker images...\n\n"
	@python3 scripts/mcp_pull_images.py --dry-run

# Pull all stdio MCP Docker images declared in mcp.json files
mcp-pull:
	@printf "\n[MCP] Pulling stdio MCP Docker images...\n\n"
	@python3 scripts/mcp_pull_images.py
	@printf "\n[MCP] Done. Starting services...\n"
...
services-up:
	@printf "\n[Services] Starting all infrastructure...\n\n"
	@docker compose up -d
	...
	@printf " ready!\n"
	@printf "\nPostgres:   postgresql://agent:localdev@localhost:5432/agent\n"

	@printf "Milvus:     localhost:19535\n"
	@printf "RabbitMQ:   internal only (amqp://agent:localdev@rabbitmq:5672)\n"
	@printf "Retrieval:  internal only (http://retrieval:8001)\n"

# Stop all services
services-down:
	@printf "\n[Services] Stopping all infrastructure...\n\n"
	@docker compose down
...
_lint-py:
	@printf "\n[Lint] Python (ruff)...\n"
	@cd services/sagent && uv run ruff check . --fix
...
_fmt-py:
	@printf "\n[Format] Python (ruff)...\n"
	@cd services/sagent && uv run ruff format .
...
docker-build-backend:
	...
	@printf "\n"
	docker build --platform linux/amd64 $(DOCKER_CACHE_FLAG) -t $(BACKEND_IMAGE) ./services/sagent
...
CLOUD_RUN_BACKEND = agent-backend
CLOUD_RUN_FRONTEND = agent-frontend
