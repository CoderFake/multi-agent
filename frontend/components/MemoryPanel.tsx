...
export function MemoryPanel({ token, isOpen, onClose }: MemoryPanelProps) {
    ...
    const handleSearch = async () => {
        ...
        setLoading(true);
        try {
            const data = await api.searchMemories(searchQuery, token);
    ...
    const handleDelete = async (memoryId: string) => {
        try {
            ...
            if (searchResults) setSearchResults(prev => prev!.filter(m => m.id !== memoryId));
        } catch (err) {
        ...
        <div className="fixed inset-y-0 right-0 w-[380px] bg-white border-l border-gray-200 shadow-xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100" style={{ background: 'linear-gradient(to right, #f0f0ff, #ededff)' }}>
                ...
                <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" onClick={fetchMemories}
                        className="h-8 w-8 text-gray-500 hover:text-[#6766FC]" title="Refresh">
            ...
            <div className="px-4 py-3 border-b border-gray-100">
                <div className="flex gap-2">
                    ...
                    <Button onClick={handleSearch} size="sm" className="text-white px-3" style={{ backgroundColor: '#6766FC' }}>
                        Search
                    </Button>
                </div>
                {searchResults && (
                    <button onClick={() => { setSearchResults(null); setSearchQuery(''); }}
                        className="mt-2 text-xs hover:underline" style={{ color: '#6766FC' }}>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
                {loading && displayList.length === 0 ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Memories are created from conversations</p>
                    </div>
                ) : (
                    displayList.map((item) => (
                        <div key={item.id}
                            ...
                            <p className="text-sm text-gray-700 pr-8 leading-relaxed">{item.memory}</p>
                            <div className="flex items-center gap-2 mt-2">
                                {item.score && <span className="text-xs text-gray-400">Score: {(item.score * 100).toFixed(0)}%</span>}
                                {item.created_at && <span className="text-xs text-gray-400">{new Date(item.created_at).toLocaleDateString()}</span>}
                                {item.categories?.map(cat => (
                                    <span key={cat} className="text-xs px-1.5 py-0.5 rounded" style={{ backgroundColor: '#ededff', color: '#6766FC' }}>{cat}</span>
                                ))}
                            </div>
            ...
            {memories.length > 0 && (
                <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
                    <Button variant="ghost" size="sm" onClick={handleDeleteAll}
                        ...
                        <Trash2 className="h-3.5 w-3.5 mr-1" />Delete All Memories
                    </Button>
                </div>
            )}
        </div>
