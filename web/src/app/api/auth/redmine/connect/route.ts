...
export async function POST(request: Request) {
    ...
    try {
        ...
        const { url, username, apiKey } = body;

        if (!url || !username || !apiKey) {
            return NextResponse.json({ error: "Missing required fields: url, username, apiKey" }, { status: 400 });
        }
        ...
        // Check if a Redmine connection already exists
        ...
        console.error("Redmine connect error:", err);
        return NextResponse.json({ error: "Failed to save Redmine connection" }, { status: 500 });
    }
}
