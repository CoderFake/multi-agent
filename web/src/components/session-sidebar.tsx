...
const clearDraft = (sessionId: string) => {
  if (typeof window !== "undefined") {
    localStorage.removeItem(`agent:draft:${sessionId}`);
  }
};
      ...
      try {
        ...
        await Promise.all(
          sessions.map((session) =>
            fetchWithAuth(`/api/sessions/${session.id}`, {
              method: "DELETE",
            }),
          ),
        );
            ...
            <SidebarMenu>
              <SidebarMenuItem>
                <div className="flex w-full items-center justify-between">
                  ...
                  >
                    <span className="cursor-pointer overflow-visible rounded-md pr-3 pl-2 transition-colors hover:bg-accent flex items-center">
                      <span className="text-xl font-bold tracking-tight bg-gradient-to-r from-blue-500 to-violet-500 bg-clip-text text-transparent">AI Agent</span>
                    </span>
                  </button>
